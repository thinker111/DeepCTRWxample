...
if __name__ == "__main__":
    ...
    data['label_income'] = data['income_50k'].map({' - 50000.': 0, ' 50000+.': 1}) # 新建一列
    data['label_marital'] = data['marital_stat'].map(lambda x: 1 if x == ' Never married' else 0) # 此处apply替换为map 结果不变
    data.drop(labels=['income_50k', 'marital_stat'], axis=1, inplace=True) #  ['label_income'] ，['label_marital']都在
    ...
    model = PLE(dnn_feature_columns, tower_dnn_hidden_units=[], task_types=['binary', 'binary'],
                 task_names=['label_income', 'label_marital']) # X，y混在一起
    ...
    print("test income AUC", round(roc_auc_score(test['label_income'], pred_ans[0]), 4)) # 保留4位小数
    print("test marital AUC", round(roc_auc_score(test['label_marital'], pred_ans[1]), 4))
   # Name       PLE    MMOE
    # income  0.2255  0.2402
    # marital 0.7468  0.7801
